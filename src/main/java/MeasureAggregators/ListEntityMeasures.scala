...
//      val entitySet = totalEntitySet.filter(_.split("\t")(3).contains(item)).cache()
      ...
      sc.parallelize(myListTemp).repartition(1).saveAsTextFile(item + "/popularity_" + item + "_" + args(0) + "_" + args(1))
...
////////
      ...
      approvedEntities.repartition(1).saveAsTextFile(item + "/K-Network_" + item + "_" + args(0) + "_" + args(1))

      val E = calculateEplusEminus(entitySet, entitySetForTopK, e1Cnt, delta, topK)
      sc.parallelize(E._1).repartition(1).saveAsTextFile(item + "/EplusNetwork_"  + item + "_" + args(0) + "_" + args(1))
      sc.parallelize(E._2).repartition(1).saveAsTextFile(item + "/EminusNetwork_" + item + "_" + args(0) + "_" + args(1))
/////
